...
SELECT COUNT(id) as TaskCount FROM task WHERE due_date IS NULL;
...
SELECT count(task.id), status.name FROM task INNER JOIN status ON  status.id = task.status_id GROUP BY task.status_id;

/*Get the names of all statuses, sorted by the status with most tasks first*/
SELECT status.name, COUNT(task.id) as TaskCount FROM task INNER JOIN status ON status.id = task.status_id 
